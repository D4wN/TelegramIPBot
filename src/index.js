...
if (!ubiSecret) {
    if (process.argv[3]) {
        ubiSecret = process.argv[3];
    } else {
        console.log("No Ubisoft 2FA Secret specified.");
    }
}
...
const helpText = "" +
    ...
    "/ubi : get current ubisoft 2FA code\n" +
...
//Ubisoft 2FA
var ubi = require('speakeasy');
class Ubisoft2FAController extends TelegramBaseController {
    ubiHandle($) {
        let user = $._update._message._from;
        let debugName = "[/ubi]";
        console.log(debugName + "Request from " + userData(user));
        if (ubiSecret == null) {
            $.sendMessage('Ubisoft2FAController: No Ubisoft Secret defined!');

        } else {
            let oneTimeCode = ubi.totp({
                'secret': ubiSecret,
                'encoding': "base32"
            });
            $.sendMessage('Ubisoft One-time Code: ' + oneTimeCode);
        }
    };
...
tg.router
    ...
    .when(['/ubi'], new Ubisoft2FAController())
